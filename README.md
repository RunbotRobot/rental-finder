...
4. Drops Craigslist "rooms & shares" listings once actually read (detail
   ...
   alone means self-contained). Explicit roommate/shared-space language
   ("roommate," "housemate," "shared kitchen/bath," "other tenants," ...)
   always wins over a self-contained-sounding signal, checked first, even
   from the title alone — a room described as having "its own entrance,
   kitchen, laundry and bathroom" doesn't mean much once the same
   description says that bathroom is shared with 3 other roommates. See
   `room_share_filter.py`. A listing with no description or attributes at
   all yet is kept until a future run actually reads it, rather than
   guessed at from the title alone.
